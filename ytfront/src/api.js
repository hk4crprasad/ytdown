...
export const api = {
  ...
  startMerge:   (url, params={}) => { const p = new URLSearchParams({ url, ...params }); return _post(`/merge/start?${p}`); },
  ...
  progressSSE:  (jobId, onEvent, onDone) => {
    ...
    src.onmessage = (e) => {
      ...
      if (data._eof || data.status === 'done' || data.status === 'error') {
        src.close(); onDone?.(data);
      }
    };
    src.onerror = () => { src.close(); onDone?.({ status: 'error', error: 'Connection lost' }); };
    return src;
  },
};
